...
= 1.5.0 =
* New (Envato-style discovery): AEC software-compatibility tags (Revit/Dynamo/Grasshopper/IFC/Excel...). Vendors tag listings; product pages show compatibility badges; the shop gets "Works with" filter pills.

= 1.4.0 =
* New (Envato-style item depth): vendors can add a Version, Changelog and Live-Preview URL to a program; product pages show version/last-updated, a Live Preview button, and a Changelog tab.
...
= 1.3.0 =
* New: [wpaec_vendors] shortcode renders a browsable directory of approved vendor stores (vendor-first discovery).

= 1.2.2 =
* Fix: vendor downloadable-file uploads could be rejected with "not located within an approved directory" when WooCommerce's approved-download-directories list was empty; the uploads directory is now auto-registered before a vendor file is attached.

= 1.2.1 =
* New: per-user rate limiting (runs/minute and runs/day) on the AI tools to guard against runaway loops and API-cost spikes; configurable under AEC Market -> Forge Tools.
* New: credit terms note on the pricing page (credits are non-expiring, charged only on success).
